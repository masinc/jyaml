...
class Lexer:
    ...
    def read_multiline_string(self, indicator: str) -> str:
        ...
        if self.current_char() != '\n':
            raise LexerError("Multiline string must start on new line", self.line, self.column)
        self.advance()
    ...
    def next_token(self) -> Token:
        ...
        if self.at_line_start:
            ...
            indent = self.count_indent()
            if indent > 0:
                return Token(type=TokenType.INDENT, value=str(indent), line=self.line, column=self.column - indent)
        ...
        if char == '#':
            ...
            return Token(type=TokenType.COMMENT, value=comment, line=line, column=column)
        ...
        if char.isalpha():
            ...
            if value == 'true':
                ...
                return Token(type=TokenType.FALSE, value=value, line=line, column=column)
            elif value == 'null':
        ...
        if char in single_char_tokens:
            self.advance()
            return Token(type=single_char_tokens[char], value=char, line=line, column=column)
