...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let custom = SerializeOptions::builder()
        ...
        .pretty(true)
        .build();
    println!("   Custom (sorted keys, 4-space indent):\n{}", to_string_with_options(&user, &custom)?);
...
"name": "Bob""#;
    ...
    println!("   Builder serialization: {}", to_string_with_options(&user, &serialize_opts)?);
...
"settings": {}"#;
    ...
    println!("   Builder deserialization: {} (age: {})", test_result.name, test_result.age);
    ...
    let lf_opts = SerializeOptions::builder().line_ending(LineEnding::Lf).pretty(true).build();
    let crlf_opts = SerializeOptions::builder().line_ending(LineEnding::Crlf).pretty(true).build();
    let none_opts = SerializeOptions::builder().line_ending(LineEnding::None).pretty(true).build();
    
    println!("   LF endings: {:?}", to_string_with_options(&line_data, &lf_opts)?.chars().filter(|&c| c == '\n' || c == '\r').collect::<Vec<_>>());
    println!("   CRLF endings: {:?}", to_string_with_options(&line_data, &crlf_opts)?.chars().filter(|&c| c == '\n' || c == '\r').collect::<Vec<_>>());
    println!("   No normalization: {:?}", to_string_with_options(&line_data, &none_opts)?.chars().filter(|&c| c == '\n' || c == '\r').collect::<Vec<_>>());
