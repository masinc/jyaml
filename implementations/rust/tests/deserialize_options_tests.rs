use jyaml::{from_str_with_options, parse_with_options, DeserializeOptions, Value, Error};
...
"name": "Bob""#;
    ...
    let permissive_options = DeserializeOptions::default()
...
fn test_max_depth_limit() {
  ...
  "level2":
    ...
    let shallow_options = DeserializeOptions::default()
        .with_max_depth(1);
    let result = parse_with_options(input, &shallow_options);
    assert!(result.is_err());
    
    // Test with sufficient max depth
    let deep_options = DeserializeOptions::default()
        .with_max_depth(10);
...
fn test_parse_options_builder_validation() {
    ...
    let valid_options = DeserializeOptions::builder()
        .max_depth(1000)
        .try_build();
    assert!(valid_options.is_ok());
    
    // Test builder with invalid options
    let invalid_options = DeserializeOptions::builder()
        .max_depth(0)
        .try_build();
    assert!(invalid_options.is_err());
    
    let too_deep_options = DeserializeOptions::builder()
        .max_depth(200000)
        .try_build();
