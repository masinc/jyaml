use jyaml::{
    ...
    to_string_with_options, DeserializeOptions, DeserializeOptionsBuilder, Error, LineEnding,
    OutputStyle, QuoteStyle, SerializeOptions, SerializeOptionsBuilder, Value,
};
...
fn create_test_user() -> TestUser {
    ...
    metadata.insert(
        "version".to_string(),
        Value::Number(jyaml::value::Number::Integer(1)),
    );
...
fn test_unicode_handling() {
    ...
    let no_escape_opts = SerializeOptions::builder().escape_unicode(false).build();
    ...
    let escape_opts = SerializeOptions::builder().escape_unicode(true).build();
...
fn test_value_api() {
    ...
    let simple_value = Value::Object(
        [
            ...
            (
                "number".to_string(),
                Value::Number(jyaml::value::Number::Integer(42)),
            ),
...
fn test_depth_limits() {
    ...
    let high_depth = DeserializeOptions::builder().max_depth(10).build();
    let result = parse_with_options(deep_input, &high_depth);
    assert!(result.is_ok());

    // Should fail with insufficient depth
    let low_depth = DeserializeOptions::builder().max_depth(2).build();
