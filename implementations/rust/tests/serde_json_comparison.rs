...
fn jyaml_to_json(jyaml_value: &JyamlValue) -> JsonValue {
    match jyaml_value {
        ...
        JyamlValue::Number(n) => match n {
            ...
            jyaml::value::Number::Float(f) => JsonValue::Number(serde_json::Number::from_f64(*f).unwrap()),
        },
        JyamlValue::String(s) => JsonValue::String(s.clone()),
        JyamlValue::Array(arr) => {
            JsonValue::Array(arr.iter().map(jyaml_to_json).collect())
        },
...
fn test_json_compatibility_basic_values() {
    ...
    for (jyaml_input, json_input) in test_cases {
        ...
        assert_eq!(converted_jyaml, json_value, "Values should match for input: {}", jyaml_input);
    }
}
...
fn test_json_compatibility_arrays() {
    ...
    for (jyaml_input, json_input) in test_cases {
        ...
        let converted_jyaml = jyaml_to_json(&jyaml_value);
        assert_eq!(converted_jyaml, json_value, "Arrays should match for input: {}", jyaml_input);
    }
}
...
fn test_json_compatibility_objects() {
    let test_cases = vec![
        ...
        (r#"{"age": 30, "active": true}"#, r#"{"age": 30, "active": true}"#),
    ...
    for (jyaml_input, json_input) in test_cases {
        ...
        let converted_jyaml = jyaml_to_json(&jyaml_value);
        assert_eq!(converted_jyaml, json_value, "Objects should match for input: {}", jyaml_input);
    }
}
...
fn test_json_compatibility_unicode() {
    ...
    for (jyaml_input, json_input) in test_cases {
        ...
        if let (JsonValue::String(jyaml_str), JsonValue::String(json_str)) = (&converted_jyaml, &json_value) {
            assert_eq!(jyaml_str, json_str, "Unicode strings should have same content for: {} vs {}", jyaml_input, json_input);
        } else {
            assert_eq!(converted_jyaml, json_value, "Values should match for input: {} vs {}", jyaml_input, json_input);
        }
    }
}
...
fn test_jyaml_surrogate_pair_parsing() {
    ...
    for (surrogate_input, expected_emoji) in test_cases {
        println!("Testing surrogate pair: {} -> {}", surrogate_input, expected_emoji);
        
        // Parse with JYAML
        let jyaml_value = from_str::<JyamlValue>(surrogate_input).unwrap();
        if let JyamlValue::String(s) = jyaml_value {
            assert_eq!(s, expected_emoji, "JYAML should convert surrogate pair to emoji");
        } else {
        ...
        if let JsonValue::String(s) = json_value {
            assert_eq!(s, expected_emoji, "JSON should also convert surrogate pair to emoji");
        } else {
            panic!("Expected string value");
        }
    }
...
fn test_json_compatibility_nested_structures() {
    ...
    let converted_jyaml = jyaml_to_json(&jyaml_value);
    assert_eq!(converted_jyaml, json_value, "Complex nested structures should match");
}
...
fn test_jyaml_specific_features_vs_json() {
    ...
    if let (Ok(jyaml_val), Ok(json_val)) = (jyaml_result, json_result) {
        ...
        assert_eq!(converted_jyaml, json_val, "Content should be identical despite comments");
    }
}
...
fn test_performance_comparison_basic() {
    ...
    assert_eq!(converted_jyaml, json_value, "Both parsers should produce equivalent results");
}
...
fn test_error_handling_comparison() {
    ...
    for (i, input) in invalid_inputs.iter().enumerate() {
        ...
        if i == 3 {
            ...
            assert!(jyaml_result.is_err(), "JYAML should reject invalid input: {}", input);
            assert!(json_result.is_err(), "JSON should reject invalid input: {}", input);
        }
    }
}
...
fn test_direct_deserialization_comparison() {
    ...
    assert!(jyaml_result.is_ok(), "JYAML should deserialize successfully");
    ...
    assert_eq!(jyaml_company, json_company, "Both parsers should produce identical structs");
...
fn test_surrogate_pair_deserialization_comparison() {
    ...
    let surrogate_data = r#"{"rocket": "\uD83D\uDE80", "party": "\uD83C\uDF89", "crab": "\uD83E\uDD80"}"#;
...
fn test_nested_collection_deserialization() {
    ...
    assert_eq!(jyaml_result.matrix, vec![vec![1, 2], vec![3, 4], vec![5, 6]]);
    assert_eq!(jyaml_result.map.get("fruits"), Some(&vec!["apple".to_string(), "banana".to_string()]));
